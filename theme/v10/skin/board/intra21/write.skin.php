...
<section id="bo_w">
    ...
    <div class="write_div">
        ...
        <input type="hidden" name="ct_id" value="<?=$write['ct_id']?>"><!-- 장바구니번호 -->
        <input type="hidden" name="com_idx" value="<?=$write['com_idx']?>"><!-- 업체번호 -->
        <input type="hidden" name="com_name" value="<?=$write['wr_1']?>"><!-- 업체명 -->
        <input type="hidden" name="mb_id_saler" value="<?=$write['mb_id_saler']?>"><!-- 영업자아이디 -->
        <input type="hidden" name="mb_name_saler" value="<?=$write['mb_name_saler']?>"><!-- 영업자명 -->
        <input type="hidden" name="trm_idx_department_saler" value="<?=$write['wr_7']?>"><!-- 영업자조직코드 -->
        <input type="hidden" name="trm_name_department_saler" value="<?=$write['trm_name_department_saler']?>"><!-- 영업자조직명 -->
        <input type="hidden" name="mb_intra_saler" value="<?=$write['mb_intra_saler']?>"><!-- 기존인트라 -->
        <input type="hidden" name="mb_intra_id" value="<?=$write['mb_intra_id']?>"><!-- 기존인트라아이디 -->
        <div style="display:<?php if($w=='u'&&!$member['mb_manage_yn']) echo 'none';?>;">
            <button type="button" class="btn btn_b01" id="btn_site">사이트찾기</button>
            <div style="display:inline-block;">인트라1, 인트라2, 인트라3, 인트라3.1 에서도 사이트 정보를 가지고 올 수 있습니다.</div>
        </div>
    </div>
...
$(function(){
	...
	$("#btn_site").click(function(e) {
		e.preventDefault();
		var url = g5_user_admin_url+"/site_select.popup.php?frm=fwrite&file_name=<?php echo $g5['file_name']?>";
		win_site_select = window.open(url, "win_site_select", "left=300,top=150,width=720,height=600,scrollbars=1");
        win_site_select.focus();
	});
