...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <form name="fsearch" method="get">
        ...
        <div class="bo_sch_group">
            ...
            <select name="sch_mb_asign_worker" id="sch_mb_asign_worker">
                <option value="">할당선택 &nbsp;</option>
                <option value="pending">미할당</option>
                <option value="asigned">할당</option>
            </select>
            <script>$('#sch_mb_asign_worker').val('<?php echo $sch_mb_asign_worker;?>').attr('selected','selected');</script>

            <select name="sch_wr_5" id="sch_wr_5" style="display:<?php if(!$member['board_manager_yn']) echo 'none';?>;">
                <option value="">작업등급전체 &nbsp;</option>
                <?php echo $g5['set_maintain_grades_options'];?>
            </select>
            ...
            <select name="sfl" id="sfl">
                ...
                <option value="mb_name_worker"<?php echo get_selected($sfl, 'mb_name_worker', true); ?>>작업자</option>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="col" style="width:60px;display:<?php if(!$member['board_manager_yn']) echo 'none';?>;">작업등급</th>
            <th scope="col" style="width:90px;">작업자/완료일</th>
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> status_<?php echo $list[$i]['wr_10'];?>">
            ...
            <td class="td_subject" style="padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '0'; ?>px"><!-- 제목 -->
                <div class="bo_tit">
            ...
            <td class="td_work_level sv_use" style="display:<?php if(!$member['board_manager_yn']) echo 'none';?>;"><!-- 작업등급 -->
                <?php echo $list[$i]['wr_5'] ?>
            </td>
            <td class="td_worker sv_use"><!-- 작업자/완료일 -->
                <?php
                if($list[$i]['wr_6']) {$list[$i]['wr_schedule_date']='<br><span class="schedule_date">~'.$list[$i]['wr_6'].'</span>';}
                ?>
                <?php echo $list[$i]['sried']['mb_name_worker'] ?>
                <?php echo $list[$i]['wr_schedule_date'] ?>
            </td>
            <td class="td_status sv_use"><!-- 상태 -->
                <?php echo $g5['set_maintain_status_value'][$list[$i]['wr_10']] ?>
            </td>
