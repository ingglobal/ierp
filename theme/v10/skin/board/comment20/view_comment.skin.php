...
for ($i=0; $row=sql_fetch_array($result); $i++) {
	if (!strstr($row['wr_option'], "secret") || $is_admin || ($row['mb_id']==$member['mb_id'] && $member['mb_id'])) {
		...
		$list[$i]['commentfile'] = get_file($board['bo_table'], $row['wr_id']);
		//print_r2($list[$i]['commentfile']);
		if(!$list[$i]['commentfile'][0]['view']){ 
			/* 이미지가 아닌건 허용된확장자만 링크 */
			//$sPermissionExt = "txt|csv|zip|tar";
			//if( preg_match("/\.($sPermissionExt)$/i", $list[$i]['commentfile'][0]['source'])) {
			if( $list[$i]['commentfile'][0]['source'] && !preg_match("/삭제됨/i",$list[$i]['commentfile'][0]['bf_content']) ) {
				$list[$i]['commentfile'][0]['view'] = "<img src='{$board_skin_url}/img/icon_file.gif' align=absmiddle border='0'> <a href='".$list[$i]['commentfile'][0]['href']."'>".$list[$i]['commentfile'][0]['source']."</a><br>";
...
<section id="bo_vc">
    ...
    <article id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;border-top-color:#e0e0e0"<?php } ?>>
        ...
        <!-- 댓글 출력 -->
        <div class="comment_file" style="display:<?php if(!$list[$i]['commentfile'][0]['view']) echo 'none';?>">
            <input type="hidden" id="commentfile_<?=$comment_id?>" value="<?=$list[$i]['commentfile'][0]['file']?>">
            <?
			// 댓글 첨부파일
			//print_r2($list[$i]);
            if($list[$i]['commentfile'][0]['view'])
            	echo $list[$i]['commentfile'][0]['view'];
            ?>
        </div>
...
<aside id="bo_vc_w" class="bo_vc_w">
    ...
    <input type="hidden" name="ser_prj_idx" value="<?php echo $ser_prj_idx ?>">
    ...
    <!-- 첨부파일 -->
    <div class="bo_comment_file">
        <input type="file" name="wr_commentfile" style="display:inline-block;">
        <label><span id="delspan" style='display:none'><input type="checkbox" name="wr_commentfile_del" value="Y"> 파일삭제</span></label>
    </div>
...
function comment_box(comment_id, work)
{
	...
	// 수정 버튼 클릭 시 파일 삭제 체크 버튼 보임
    if (comment_id){
	    if(document.getElementById(cf_id).value && work =='cu') {
			document.getElementById('delspan').style.display = "inline-block";
	    }
        else {
			document.getElementById('delspan').style.display = "none";
        }
    }
}
