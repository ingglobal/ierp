...
<?php include($board_skin_path.'/header.php');?>
...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <div id="bo_btn_top">
        ...
        <ul class="btn_bo_user">
            ...
            <?php if ($is_admin=='super') { ?><li style="display:<?php if(auth_check($auth[$sub_menu],"d",1)) echo 'none';?>"><a href="<?php echo $board_skin_url ?>/config_form.php?bo_table=<?php echo $bo_table;?>" class="btn_admin btn"><i class="fa fa-gear" aria-hidden="true"></i> 환경설정</a></li><?php } ?>
            <?php if ($is_admin=='super') { ?><li style="display:<?php if(auth_check($auth[$sub_menu],"d",1)) echo 'none';?>"><a href="<?php echo $admin_href ?>" class="btn_admin btn"><i class="fa fa-user-circle" aria-hidden="true"></i> 관리자</a></li><?php } ?>
            <li><a href="javascript:window.close();" class="btn btn_02"><i class="fa fa-tiles" aria-hidden="true"></i> 창닫기</a></li>
            <?php if ($list_href) { ?><li><a href="<?php echo $list_href ?>&ser_prj_idx=<?=$ser_prj_idx?>" class="btn_b01 btn"><i class="fa fa-list" aria-hidden="true"></i> 목록</a></li><?php } ?>
    ...
    <form name="fsearch" method="get">
        ...
        <input type="hidden" name="ser_prj_idx" value="<?=$ser_prj_idx?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="col" style="width:60px;display:<?php if(!in_array($member['mb_id'],$board['grade_view_ids'])) echo 'none';?>;">작업등급</th>
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> status_<?php echo $list[$i]['wr_10'];?>">
            ...
            <td class="td_subject" style="padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '5'; ?>px"><!-- 제목 -->
                <div class="bo_tit">
                ...
                <?php
                // 구매상품(장바구니) 정보
                if($list[$i]['sried']['ct_id']) {
                    //print_r3($list[$i]['sried']['ct_id']);
                    $ct1 = get_table_meta('g5_shop_cart','ct_id',$list[$i]['sried']['ct_id'],'shop_cart');
                    //print_r3($ct1);
                ...
                ?>
                <div class="item_info">
                    <span class="item_name"><?php echo $ct1['it_name']?></span>
                    <span class="item_price"><?php echo number_format($ct1['ct_price_total'])?>원</span>
                    <span class="item_status cart_status_<?php echo $ct1['ct_status_class']?>"><b>상태:</b> <?php echo $ct1['ct_status']?></span>
                </div>
                <?php } ?>
                <?php
                // 업체정보
                if($list[$i]['sried']['com_idx']) {
                    $com1 = get_table_meta('company','com_idx',$list[$i]['sried']['com_idx']);
                    //print_r3($com1);
                ?>
                <div class="company_info">
                    <span class="com_name"><b>업체:</b><?php echo $com1['com_name']?></span>
                    <span class="mb_name_saler"><b>영업자:</b><?php echo $ct1['mb_name_saler_rank']?>(<?php echo $g5['department_name'][$list[$i]['wr_7']]?>)</span>
                </div>
                <?php } ?>
            </td>
            <td class="td_wr_name sv_use"><?php echo $list[$i]['name'] ?></td>
            <td class="td_datetime"><?php echo $list[$i]['datetime2'] ?></td>
            <td class="td_work_level sv_use" style="display:<?php if(!in_array($member['mb_id'],$board['grade_view_ids'])) echo 'none';?>;"><!-- 작업등급 -->
