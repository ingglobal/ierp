...
<div class="local_ov01 local_ov">
    ...
    <form name="fsearch" method="get" onsubmit="return fsearch_submit(this);">
        ...
        <div class="bo_sch_group">
            ...
            <select name="sfl" id="sfl">
                ...
                <option value="wr_subject"<?php echo get_selected($sfl, 'wr_subject', true); ?>>특근사유</option>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <?php } ?>
            <th scope="col">번호</th>
            <th scope="col">특근사유</th>
            <th scope="col">신청자</th>
            <th scope="col">부서</th>
            <th scope="col">승인자</th>
            <th scope="col">시작일시</th>
            <th scope="col">특근시간</th>
            <?php if($is_infoauth) { ?><th scope="col">특근수당</th><?php } ?>
            <th scope="col">상태</th>
        </tr>
        </thead>
        <tbody>
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> <?php echo $lt_class ?>">
            ...
            <td class="td_name sv_use">
                <?php
                //echo $list[$i]['wr_mb_id_worker']
                $wmb = get_member($list[$i]['wr_mb_id_worker'],'mb_name');
                echo $wmb['mb_name'];
                ?>
            </td>
            <td class="td_part">
                <?php
                echo $g5['set_department_name_value'][$list[$i]['wr_mb_part']];
                ?>
            </td>
            <td class="td_name">
                <?php
            ...
            <td class="td_datetime">
                <?php
                $worktime_arr = explode(' ',$list[$i]['wr_work_dt']);
                $list[$i]['wr_work_date'] = $worktime_arr[0];
                $list[$i]['wr_work_time'] = $bo_time_value[$worktime_arr[1]];
                echo $list[$i]['wr_work_date']." / ";
                echo $list[$i]['wr_work_time'];
                ?>
            </td>
            <td class="td_hours">
                <?=$list[$i]['wr_hour_count']?>시간
            </td>
            <?php if($is_infoauth) { 
                $mbprc = get_member($list[$i]['wr_mb_id_worker'],'mb_7');
                $hours_acc += $list[$i]['wr_hour_count'];
                $price_acc += $mbprc['mb_7'] * $list[$i]['wr_hour_count'] * $bo_exrate_value[$list[$i]['wr_work_type']];
            ?>
                <td class="td_price"><?php echo number_format($mbprc['mb_7']).' x '.$list[$i]['wr_hour_count'].'h x '.$bo_exrate_value[$list[$i]['wr_work_type']].' = '.number_format($mbprc['mb_7'] * $list[$i]['wr_hour_count'] * $bo_exrate_value[$list[$i]['wr_work_type']]); ?></td>
            <?php } ?>
            <?php
                $blink_class = '';
            ...
            ?>
            <td class="td_status<?=$blink_class?>">
                <?=$g5['set_approve_status_value'][$list[$i]['wr_apply_status']]?>
            </td>

        </tr>
        <?php if($i == (count($list) - 1) && ($member['mb_level'] >= 8 || $member['mb_6'] < 2)){ ?>
        <tr>
        <td colspan="7" class="td_tot">합계</td>
        <td class="td_tot td_h"><?=number_format($hours_acc)?>시간</td>
        <td class="td_tot td_p"><?=number_format($price_acc)?>원</td>
        <td class="td_tot"></td>
        </tr>
