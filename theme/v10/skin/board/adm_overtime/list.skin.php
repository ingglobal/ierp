...
<div class="local_ov01 local_ov">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> <?php echo $lt_class ?>">
            ...
            <td class="td_type">
                <?php
                echo $bo_type_value[$list[$i]['wr_work_type']];
                ?>
            </td>
        ...
        <tr>
        <td colspan="<?=($colspan-1)?>" class="td_tot">합계</td>
        <td class="td_tot td_h"><?=number_format($hours_acc)?>H</td>
        <td class="td_tot"></td>
        </tr>
        <?php } //if($i == (count($list) - 1)) ?>
        <?php } //for($i=0; $i<count($list); $i++) ?>
        <?php 
        $capn = ($member['mb_level'] >= 8 || $member['mb_6'] < 2) ? $colspan+2 : $colspan+1;
        if (count($list) == 0) { echo '<tr><td colspan="'.$capn.'" class="empty_table">게시물이 없습니다.</td></tr>'; } 
        ?>
        </tbody>
        </table>
    </div>
