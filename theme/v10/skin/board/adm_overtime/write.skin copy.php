...
<section id="bo_w">
    ...
    <div class="ovt_info">
        <ul class="ul_info">
            ...
            <?php if($w == 'u' && ($member['mb_6'] <= 2 || $member['mb_level'] >= 8) && $member['mb_id'] != $write['wr_mb_id_worker']){
                $minf = sql_fetch(" SELECT mb_7 FROM {$g5['member_table']} WHERE mb_id = '{$write['wr_mb_id_worker']}' ");
            ?>
            <li class="li_info li_info100 li_info_left" style="display:none;">
            기본시급 : <?=number_format($minf['mb_7'])?>원
            </li>
            <li class="li_info li_info100 li_info_left">
            수당시급 : <?=number_format($minf['mb_7'])?>원 x <?=$bo_exrate_value[$write['wr_work_type']]?> = <?=number_format($write['wr_hour_price'])?>원
            </li>
            <li class="li_info li_info100 li_info_left">
            총지급액 : <?=number_format($write['wr_hour_price'])?>원 x <?=$bo_hours_value[$write['wr_hour_count']]?> = <?=number_format($write['wr_total_price'])?>원
            </li>
            <?php } ?>
        </ul>
    </div>
