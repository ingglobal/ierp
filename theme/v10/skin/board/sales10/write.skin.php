...
<section id="bo_w">
    ...
    <div class="write_div">
        ...
        <input type="hidden" name="mb_name_customer" value="<?=$write['wr_4']?>"><!-- 고객명 -->
        <div style="display:<?php if($w=='u'&&!$member['mb_manager_yn']) echo 'none';?>;">
            ...
            담당자가 존재하지 않는 경우 업체등록 및 담당자등록을 먼저 하신 후 영업등록을 할 수 있습니다. <a href="<?=G5_USER_ADMIN_URL?>/member_list.php">[담당자등록바로가기]</a>
        </div>
        <!-- 고객(업체)정보 -->
        <div class="customer_info" style="display:<?=(!$write['customer_info'])?'none':''?>;">
            <?=$write['customer_info']?>
        </div>
    </div>

    <div class="bo_user write_div" style="display:<?=(!$member['mb_manager_yn'])?'none':''?>;">

        <label for="wr_5" class="sound_only">작업등급</label>
        <select name="wr_5" id="wr_5" class="frm_input">
            <option value="">관심등급 선택</option>
    ...
    <div class="btn_fixed_top">
        <a href="javascript:history.back();" class="btn_cancel btn" style="line-height:28px;">취소</a>
