...
?>
<style>
.select_area:after{display:block;visibility:hidden;clear:both;content:'';}
</style>
...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <form name="fsearch" method="get">
        ...
        <div class="bo_sch_date">
            ...
            <input type="text" id="fr_date" name="fr_date" value="<?php echo $fr_date; ?>" placeholder="접수일시작" class="frm_input" maxlength="10" placeholder="부터">
            ~
            <input type="text" id="to_date" name="to_date" value="<?php echo $to_date; ?>" placeholder="접수일종료" class="frm_input" maxlength="10" placeholder="까지"><br>
            </div>
            <div style="display:inline-block;margin-left:10px;">
            <input type="text" id="fr_date2" name="fr_date2" value="<?php echo $pl_date2; ?>" placeholder="방문예정시작" class="frm_input" maxlength="10" placeholder="부터">
            ~
            <input type="text" id="to_date2" name="to_date2" value="<?php echo $to_date2; ?>" placeholder="방문예정종료" class="frm_input" maxlength="10" placeholder="까지">
            </div>
        </div>
        ...
        <div class="bo_sch_group">
            ...
            <div class="sch_area">
                ...
                <input type="text" name="stx" value="<?php echo stripslashes($stx) ?>" id="stx" class="sch_input" size="25" maxlength="20" placeholder="검색어를 입력해주세요" style="width:277px !important;">
...
//            print_r2($list[$i]['cmm']);
        ...
        ?>
        <li class="<?php if ($list[$i]['is_notice']) echo " bo_notice"; ?> status_<?php echo $list[$i]['wr_10'];?><?=$bg?>">
            <?php if ($is_checkbox) { ?>
            <div class="td_chk">
            ...
            <?php } ?>
            <div class="td_com_name"><!-- 업체/고객 -->
                <span style="color:gray;">업체명:</span> <?php echo $list[$i]['com']['com_name'] ?>
                <!--span style="margin-left:10px;"><span style="color:gray;">업체담당자:</span> <?php echo $list[$i]['cmm']['cmm_name_rank'] ?></span-->
                <span style="margin-left:10px;"><span style="color:gray;">업체담당자:</span> <?php echo $list[$i]['wr_4'] ?></span>
            </div>
            <!-- 제목 [ -->
            <div class="td_subject" style="padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '0'; ?>px;">
                <div class="bo_tit">
                    ...
                    <a href="<?php echo $list[$i]['href'] ?>" style="color:#333;">
                        ...
                        <?php echo cut_str($list[$i]['subject'],20,'...'); ?>
            ...
            <div class="td_as">
                <span style="color:gray;">AS담당자:</span> <?php echo $list[$i]['wr_5'] ?>
                <span style="margin-left:10px;">
                <!--span style="color:gray;">접수경로:</span--> <?php //echo $g5['set_as_receiptpath_value'][$list[$i]['wr_9']] ?>
                <span style="color:gray;">비용:</span> <?php echo number_format($list[$i]['wr_7']); ?>
                </span>
            </div><!-- 접수경로 -->
            <div class="td_reg">
                <?php if($list[$i]['wr_homepage']){ ?><span style="color:gray;">접수:</span> <?php echo substr($list[$i]['wr_homepage'],5,5); ?><?php } ?>
                <?php if($list[$i]['wr_facebook_user']){ ?><span style="margin-left:10px;color:gray;">방문예정:</span> <?php echo substr($list[$i]['wr_facebook_user'],5,5); ?><?php } ?>

                <?php if($member['mb_6'] == 1 || $member['mb_level'] >= 8){ ?>
                <?php if($list[$i]['wr_8']){ ?><span style="margin-left:10px;color:gray;">계산서발행:</span> <?php echo substr($list[$i]['wr_8'],5,5); ?><?php } ?>
                <?php } ?>
            </div><!-- 계산서발행일 -->
            <div class="td_cost"><!--span style="color:gray;">비용:</span--> <?php //echo number_format($list[$i]['wr_7']); ?>
                <span style="color:gray;">상태:</span>
            ...
            <div class="td_modify sv_use" style="display:<?php if(!$is_admin) echo 'none';?>;"><!-- 수정 -->
                <a href="<?php echo G5_BBS_URL?>/write.php?w=u&bo_table=<?php echo $bo_table?>&wr_id=<?php echo $list[$i]['wr_id']?>&<?php echo $qstr?>"><i class="fa fa-pencil-square-o"></i></a>
            </div>
    ...
    <div class="bo_fx" style="padding-top:10px;">
        ...
        <ul class="btn_bo_user">
            <?php if ($is_checkbox) { ?>
            <li style="display:none;"><button type="submit" name="btn_submit" value="선택수정" onclick="document.pressed=this.value" class="btn btn_admin"><i class="fa fa-edit" aria-hidden="true"></i></button></li>
            <li style="display:<?php if(auth_check($auth[$sub_menu],"d",1)) echo 'none';?>"><button type="submit" name="btn_submit" value="선택삭제" onclick="document.pressed=this.value" class="btn btn_admin"><i class="fa fa-trash-o" aria-hidden="true"></i></button></li>
            <li style="display:<?php if(auth_check($auth[$sub_menu],"d",1)) echo 'none';?>"><button type="submit" name="btn_submit" value="선택복사" onclick="document.pressed=this.value" class="btn btn_admin"><i class="fa fa-files-o" aria-hidden="true"></i></button></li>
            <li style="display:<?php if(auth_check($auth[$sub_menu],"d",1)) echo 'none';?>"><button type="submit" name="btn_submit" value="선택이동" onclick="document.pressed=this.value" class="btn btn_admin"><i class="fa fa-arrows" aria-hidden="true"></i></button></li>
            <?php } ?>
            <?php if ($list_href) { ?><li><a href="<?php echo $list_href ?>" class="btn_b01 btn"><i class="fa fa-list" aria-hidden="true"></i></a></li><?php } ?>
            <?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>" class="btn_b02 btn"><i class="fa fa-pencil" aria-hidden="true"></i></a></li><?php } ?>
        </ul>
        <?php } ?>
    </div>
    <?php } ?>
