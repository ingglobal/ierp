...
<ul class="btn_top top btn_bo_user"> 
	...
	<li>
	    ...
	    <ul class="more_opt is_view_btn">
	    	<?php if (false){//($reply_href) { ?><li><a href="<?php echo $reply_href ?>"><i class="fa fa-reply" aria-hidden="true"></i> 답변</a></li><?php } ?>
	    	...
	    	<?php if (false){//($copy_href) { ?><li><a href="<?php echo $copy_href ?>" onclick="board_move(this.href); return false;"><i class="fa fa-files-o" aria-hidden="true"></i> 복사</a></li><?php } ?>
	    	<?php if (false){//($move_href) { ?><li><a href="<?php echo $move_href ?>" onclick="board_move(this.href); return false;"><i class="fa fa-arrows" aria-hidden="true"></i> 이동</a></li><?php } ?>
	    	<?php if (false){//($search_href) { ?><li><a href="<?php echo $search_href ?>">검색</a></li><?php } ?>
...
<article id="bo_v" style="width:<?php echo $width; ?>">
        ...
        <nav id="bo_cate">
            ...
            <ul id="bo_cate_ul">
                <?php for($i=0;$i<count($category_arr);$i++){
                    $cfocus = ($wr_id_cat_arr[$wr_id] == $category_arr[$i]) ? ' id="bo_cate_on"' : ''; 
                    $curl = G5_USER_ADMIN_URL.'/bbs_board.php?bo_table='.$bo_table.'&wr_id='.$cat_wr_id_arr[$category_arr[$i]];   
                ?>
                <li><a href="<?=$curl?>"<?=$cfocus?>><?=$category_arr[$i]?></a></li>
                <?php } ?>
            </ul>
        </nav>
        <?php } ?>
    ...
    <section id="bo_v_atc">
        ...
        <?php if (false){//($is_signature) { ?><p><?php echo $signature ?></p><?php } ?>

        <?php if (false){//( $good_href || $nogood_href) { ?>
        <div id="bo_v_act">
            <?php if ($good_href) { ?>
    ...
    <?php if(false){//(isset($view['link'][1]) && $view['link'][1]) { ?>
    <!-- 관련링크 시작 { -->
    ...
    <?php if (false){//($prev_href || $next_href) { ?>
    <ul class="bo_v_nb">
