...
<section id="bo_w">
    ...
    if ($is_notice || $is_html || $is_secret || $is_mail) { 
        $option = '';
        if ($is_notice) {
            $option .= PHP_EOL.'<li class="chk_box"><input type="checkbox" id="notice" name="notice"  class="selec_chk" value="1" '.$notice_checked.'>'.PHP_EOL.'<label for="notice"><span></span>운영</label></li>';
        }
        if (false){ //($is_html) {
            if ($is_dhtml_editor) {
    ...
    <div class="form_01 write_div">
        ...
        <?php if(false){ //for ($i=1; $is_link && $i<=G5_LINK_COUNT; $i++) { ?>
