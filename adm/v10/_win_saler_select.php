...
$where = array();
$where[] = " mb_level IN (6,7,8,9) ";   // 디폴트 검색조건
...
<div class="new_win">
	...
	<div id="com_sch_list" class="new_win">
		...
		<div class="tbl_head01 tbl_wrap">
			...
			?>
				<tr>
				<td class="td_mb_name" style="text-align:center;"><!-- 성명 -->
					<b><?php echo get_text($row['mb_name']); ?></b>
				</td>
				<td class="td_mb_rank" style="text-align:center;"><!-- 직책 -->
					<b><?php echo  $g5['set_mb_ranks_value'][$row['mb_3']]; ?></b>
				</td>
			...
			?>
				<tr>
					<td class='td_empty' colspan='4'>
						자료가 없습니다.<br>
						<?php if(G5_IS_MOBILE){ ?>영업자 등록은 PC버전에서 실행해 주세요.<?php } ?>
						<?php if(!G5_IS_MOBILE){ ?>
						<a href="<?=G5_USER_ADMIN_URL?>/employee_form.php" target="_blank" class="ov_listall" style="margin-top:5px;">관리자(사원)등록</a>
						<?php } ?>
					</td>
				</tr>
			<?php } ?>
...
$('.a_mag').on('click',function(){
	...
	opener.document.getElementById('mb_id_saler').value = $(this).attr('mb_id');
	opener.document.getElementById('mb_name_sl').value = $(this).attr('mb_name');
	window.close();
});
</script>
<?php
