...
$colspan = (G5_IS_MOBILE) ? 5 : 6;
?>
<style>
.new_win{position:relative;}
.new_win .btn_close{position:absolute;top:10px;right:15px;}
.td_com_tel, .td_com_president {white-space:nowrap;}
<?php if(G5_IS_MOBILE){ ?>
.btn{height:35px;line-height:35px;}
.td_mng button, .td_mng button.btn{line-height:26px;}
<?php } ?>
</style>
...
<div id="sch_target_frm" class="new_win scp_new_win">
    ...
    <div id="scp_list_find">
        ...
        <input type="text" name="sch_word" id="sch_word" value="<?php echo get_text($sch_word); ?>" class="frm_input required" required size="20" style="width:25%;">
...
//            print_r2($row);
        ?>
        <tr>
            <td class="td_mb_name" style="text-align:center;"><?=$row['mb_name']?></td>
            <td class="td_cmm_title" style="text-align:center;"><?=$g5['set_mb_ranks_value'][$row['cmm_title']]?></td>
            <td class="td_com_name"><?php echo $row['com_name']; ?></td>
            <?php if(!G5_IS_MOBILE){ ?>
            <td class="td_cmm_reg_dt"><?=substr($row['cmm_reg_dt'],2,8)?></td>
            <?php } ?>
            <td class="td_mng td_mng_s">
                <button type="button" class="btn btn_03 btn_select" style="display:<?=(!$row['com_idx'])?'none':''?>;background:#ddd;color:#333;"
        ...
        if($i ==0)
            echo '<tr><td colspan="'.$colspan.'" class="empty_table">검색된 자료가 없습니다.</td></tr>';
        ?>
        </tbody>
        </table>
    </div>
...
$('.btn_select').click(function(e){
    ...
    ?>
        ...
        opener.document.querySelector('.customer_info').style.display = 'block';
