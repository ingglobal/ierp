...
if (isset($prev['wr_id']) && $prev['wr_id']) {
    ...
    $prev_href = G5_USER_ADMIN_URL.'/bbs_board.php?bo_table='.$bo_table.'&amp;wr_id='.$prev['wr_id'].'&amp;'.$qstr;
...
if (isset($next['wr_id']) && $next['wr_id']) {
    ...
    $next_href = G5_USER_ADMIN_URL.'/bbs_board.php?bo_table='.$bo_table.'&amp;wr_id='.$next['wr_id'].'&amp;'.$qstr;
...
if ($member['mb_level'] >= $board['bo_reply_level']) {
    $reply_href = short_url_clean(G5_USER_ADMIN_URL.'/bbs_write.php?w=r&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;'.$qstr);
}
...
if ($is_member) {
    // 스크랩 링크
    $scrap_href = G5_USER_ADMIN_URL.'/bbs_scrap_popin.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id;

    // 추천 링크
    if ($board['bo_use_good'])
        $good_href = G5_USER_ADMIN_URL.'/bbs_good.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;good=good';

    // 비추천 링크
    if ($board['bo_use_nogood'])
        $nogood_href = G5_USER_ADMIN_URL.'/bbs_good.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;good=nogood';
}
