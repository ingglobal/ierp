...
$sql_common = " ca_id               = '$ca_id',
                ...
                it_10               = '$it_10',
                com_idx             = '$com_idx',
                it_buy_price        = '$it_buy_price',
                it_buy_date        = '$it_buy_date'
                ";
...
// 재고확인 문자 발송
if ($config['cf_sms_use'] == 'icode' && $it_stock_qty <= $g5['setting']['set_it_stock_min'])
{
    $send_number = preg_replace("/[^0-9]/", "", $sms5['cf_phone']);// 발신자번호
    $towhom_hp[0] = $g5['setting']['set_it_stock_hp'];
    $sms_contents = $g5['setting']['set_it_stock_sms_content'];
    $sms_contents = preg_replace("/{부품명}/", $it_name, $sms_contents);
    $sms_contents = preg_replace("/{재고량}/", $it_stock_qty, $sms_contents);
    $sms_contents = preg_replace("/{프로젝트명}/", $prj['prj_name'], $sms_contents);
    $sms_contents = preg_replace("/{종료일}/", $prj['prj_end_date'], $sms_contents);
    $sms_contents = preg_replace("/{이름}/", $mb['mb_name'], $sms_contents);
    $sms_contents = preg_replace("/{회원아이디}/", $mb['mb_id'], $sms_contents);
    $sms_contents = preg_replace("/{HOME_URL}/", '<a href="'.G5_URL.'">'.G5_URL.'</a>', $sms_contents);
    $sms_contents = preg_replace("/{DATA_AGREE_URL}/", '<a href="'.G5_USER_URL.'/e1.php?'.$com_idx.'" style="color:white;">자료사용동의하기</a>', $sms_contents);
    ...
    if($config['cf_sms_type'] == 'LMS') {
        ...
        if($port_setting !== false) {
            ...
            for($j=0;$j<sizeof($towhom_hp);$j++) {
                $sms_hp[$j] = preg_replace("/[^0-9]/", "", $towhom_hp[$j]);
                $strData[] = iconv_euckr(stripslashes($sms_contents));
                $strDest[]   =  $sms_hp[$j];
            }
        ...
        for($j=0;$j<sizeof($towhom_hp);$j++) {
            $sms_hp[$j] = preg_replace("/[^0-9]/", "", $towhom_hp[$j]);
            $sms_msg[$j] = iconv_euckr(stripslashes($sms_contents));
...
<script>
    if (confirm("계속 입력하시겠습니까?"))
        //location.href = "<?php echo "./item_sell_form.php?it_id=$it_id&amp;sort1=$sort1&amp;sort2=$sort2&amp;sel_ca_id=$sel_ca_id&amp;sel_field=$sel_field&amp;search=$search&amp;page=$page"?>";
        location.href = "<?php echo "./item_sell_form.php?".str_replace('&amp;', '&', $qstr); ?>";
    else
        location.href = "<?php echo "./item_sell_list.php?".str_replace('&amp;', '&', $qstr); ?>";
</script>
