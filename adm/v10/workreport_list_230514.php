...
if($is_admin) echo $g5['container_sub_title'];
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?=$bg?>">
        <td class="td_chk">
            <input type="hidden" name="wrp_idx[<?=$i?>]" value="<?=$row['wrp_idx']?>" id="wrp_idx_<?=$i?>">
			<label for="chk_<?=$i?>" class="sound_only"><?=get_text($row['wrp_subject'])?></label>
			<input type="checkbox" name="chk[]" ppc_idx="<?=$row['wrp_idx']?>" prj_idx="<?=$row['prj_idx']?>" value="<?=$i?>" id="chk_<?=$i?>">
        </td>
