...
if ($act_button == "선택수정"){
    ...
    foreach($chk as $idx){
        ...
        $dfres = sql_fetch("SELECT GROUP_CONCAT(DISTINCT fle_idx) AS fle_idxs FROM {$g5['file_table']}
            WHERE fle_db_table = 'drf' AND fle_type = 'drf' AND fle_db_id = '{$drf_idx[$idx]}' ");
        ...
        // drf_idx의 레코드를 삭제
        $dsql2 = " DELETE FROM {$g5['draft_table']} WHERE drf_idx = '{$drf_idx[$idx]}' ";
        sql_query($dsql2,1);
    }
    // 만약 테이블에 아무 레코드도 없으면 AUTO_INCREMENT를 1로 초기화한다.
    $cntres = sql_fetch(" SELECT EXISTS( SELECT 1 FROM {$g5['draft_table']} ) AS cnt ");
    if(!$cntres['cnt']){
        $resetsql = " ALTER TABLE {$g5['draft_table']} auto_increment=1 ";
        sql_query($resetsql);
    }
}

goto_url('./draft_list.php?'.$qstr);