...
<header id="hd">
    <h1 class="sound_only"><?php echo $config['cf_title'] ?></h1>
    <div id="hd_top">
        <button type="button" id="btn_gnb" class="">메뉴</button>
       <div id="logo"><a href="<?php echo correct_goto_url(G5_ADMIN_URL); ?>">ADMININISTOR</a></div>
    ...
    <nav id="gnb" class="gnb_large <?php echo $adm_menu_cookie['gnb']; ?>">
		...
		<div class="gnb_con">
			<h2>관리자메뉴<i class="fa fa-times gnb_con_close" aria-hidden="true"></i></h2>
			<ul class="gnb_ul">
				<?php
				$jj = 1;
...
jQuery(function($){
	...
	$('.gnb_con_close,.gnb_bg').on('click',function(){
		$('#gnb').removeClass('focus');
	});
	
	$("#btn_gnb").on('click',function(){
		if($('#gnb').hasClass('focus')){
			$('#gnb').removeClass('focus');
		}else{
			$('#gnb').addClass('focus');
		}
	...
	if($('#container_title').length){
		$('#container_title').wrap('<div id="container_title_box"></div>');
	}
	if($('#container_sub_title').length && $('#container_title_box').length){
		$('#container_sub_title').appendTo('#container_title_box');
		$('#container_title_box').appendTo('#hd');
	}else if(!$('#container_sub_title').length && $('#container_title_box').length){
		$('#container_title_box').appendTo('#hd');
	}else{
		$('#container_title_box').appendTo('#hd');
	}
	...
	if(navigator.userAgent.match(/(iPod|iPhone|iPad)/)){
		setTimeout(function(){
			cur_y = $('body').scrollTop();
		},500);
	}else{
		cur_y = $(document).scrollTop();
	}
	
	if(cur_y > stk_y) $('#hd').addClass('sticky');
	else $('#hd').removeClass('sticky');
	
	$(document).on('scroll touchmove',function(){
		if($(window).scrollTop() > stk_y) $('#hd').addClass('sticky');
		else $('#hd').removeClass('sticky');
	});
...
<div id="wrapper">
    ...
    <div id="container" class="<?php echo $adm_menu_cookie['container']; ?>">
        ...
        <div class="container_wr" style="overflow-x:hidden;">