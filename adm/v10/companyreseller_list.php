...
$sql_common = " FROM {$g5['companyreseller_table']} AS com 
                LEFT JOIN {$g5['companyreseller_member_table']} AS cmm ON cmm.com_idx = com.com_idx AND cmm_status = 'ok'
                LEFT JOIN {$g5['member_table']} AS mb ON mb.mb_id = cmm.mb_id
"; 

$where = array();
$where[] = " com_type IN ('reseller') ";   // 디폴트 검색조건
...
$sql = " SELECT SQL_CALC_FOUND_ROWS DISTINCT com.com_idx, com_name, com_names, com_type, com_level, com_reg_dt, com_status
...
//                echo $key.'='.$value.'<br>';
		...
		// 수정 및 발송 버튼
//		if($is_delete) {
			$s_mod = '<a href="./companyreseller_form.php?'.$qstr.'&amp;w=u&amp;com_idx='.$row['com_idx'].'&amp;ser_com_type='.$ser_com_type.'&amp;ser_trm_idx_salesarea='.$ser_trm_idx_salesarea.'">수정</a>';
...
//		}
	...
	<tr class="<?php echo $bg; ?> <?=$row['com_status_trash_class']?>" tr_id="<?php echo $row['com_idx'] ?>">
        ...
        <td class="td_com_type td_left"><!-- 업종 -->
            <?php echo $g5['set_com_class_value'][$row['com_type']] ?>
        </td>
        <td class="td_com_level"><!-- 업체등급 -->
            <?php echo $g5['set_com_level_value'][$row['com_level']]; ?>
        </td>
        <td class="td_com_dc_rate"><!-- 할인률 -->
            <?php echo $g5['set_com_dc_rate_value'][$row['com_level']].'%'; ?>
        </td>
...
<div class="btn_fixed_top">
    ...
    <?php } ?>
    <a href="./companyreseller_form.php" id="bo_add" class="btn_01 btn">판매업체추가</a>
</div>
...
$(function(e) {
    ...
    $(".btn_manager").click(function(e) {
        var href = "./companyreseller_member_list.php?com_idx="+$(this).attr('com_idx');
