...
// 업체정보 추출
if ($w!='')
	$com = sql_fetch(" SELECT * FROM {$g5['companyreseller_table']} WHERE com_idx = '$com_idx' ");
...
if ($w == '') {
    // 업체 정보 생성
	$sql = " INSERT into {$g5['companyreseller_table']} SET
				{$sql_common} 
...
// 수정
    ...
    $sql = "	UPDATE {$g5['companyreseller_table']} SET 
					{$sql_common}
					, com_update_dt = '".G5_TIME_YMDHIS."'
	...
	if (!$com['com_idx']) {
		...
		// 자료 삭제
		$sql = " UPDATE {$g5['companyreseller_table']} SET com_status = 'trash' WHERE com_idx = $com_idx ";
		sql_query($sql,1);
	}
	
	goto_url('./companyreseller_list.php?'.$qstr, false);
}
else
...
$del_where = (@sizeof($mb_id_saler_array))? " AND mb_id NOT IN ('".implode("','",$mb_id_saler_array)."') ":"";
$sql = " DELETE FROM {$g5['companyreseller_member_table']} WHERE com_idx='{$com['com_idx']}' $del_where ";
sql_query($sql,1);
...
for($i=0;$i<count($_FILES[$fle_type2.'_file']['name']);$i++) {
	...
	if (${$fle_type2.'_del'}[$i] == 1) {
		if($mb_id) {
			delete_jt_file(array("fle_db_table"=>"companyreseller", "fle_db_id"=>$com_idx, "fle_type"=>$fle_type2, "fle_sort"=>$i, "fle_delete"=>1));
		}
		else {
			// fle_db_id를 던져서 바로 삭제할 수도 있고 $fle_db_table, $fle_db_id, $fle_token 를 던져서 삭제할 수도 있음
			delete_jt_file(array("fle_db_table"=>"companyreseller"
								,"fle_db_id"=>$com_idx
								,"fle_type"=>$fle_type2
								,"fle_sort"=>$i
								,"fle_delete"=>1
...
for($i=0;$i<@count($_FILES[$fle_type3.'_file']['name']);$i++) {
	...
	if (${$fle_type3.'_del'}[$i] == 1) {
		if($mb_id) {
			delete_jt_file(array("fle_db_table"=>"companyreseller", "fle_db_id"=>$com_idx, "fle_type"=>$fle_type3, "fle_sort"=>$i, "fle_delete"=>1));
		}
		else {
			// fle_db_id를 던져서 바로 삭제할 수도 있고 $fle_db_table, $fle_db_id, $fle_token 를 던져서 삭제할 수도 있음
			delete_jt_file(array("fle_db_table"=>"companyreseller"
...
$r = sql_query(" desc {$g5['companyreseller_table']} ");
...
foreach($_REQUEST as $key => $value ) {
	...
	if(!in_array($key,$db_fields) && substr($key,0,3)==$db_prefix) {
		...
		meta_update(array("mta_db_table"=>"companyreseller","mta_db_id"=>$com_idx,"mta_key"=>$key,"mta_value"=>$value));
	}
}
...
// exit;
if($w == 'u') {
	//alert('업체 정보를 수정하였습니다.','./companyetc_form.php?'.$qstr.'&amp;w=u&amp;com_idx='.$com_idx, false);
	alert('업체 정보를 수정하였습니다.','./companyreseller_list.php?'.$qstr, false);
}
else {
	alert('업체 정보를 등록하였습니다.','./companyreseller_list.php', false);
}
?>