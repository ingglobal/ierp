...
if ($w == '') {
    ...
    $orp_result = sql_fetch(" SELECT prp_price FROM {$g5['project_price_table']} WHERE prj_idx = ". ${$pre.'_idx'}." AND prp_type = 'order' ");
    $ord_price = $orp_result['prp_price'];
...
$items1 = array(
    ...
    ,"prj_doc_no"=>array("발행번호","",130,0,'','',0)
    ,"prj_name"=>array("프로젝트명","required",250,0,'','',2)
    ,"mb_id_company"=>array("제출담당자","required",100,0,'아이디 입력','',0)
    ,"mb_id_saler"=>array("영업담당자","required",100,0,'아이디 입력','',2)
    ,"mb_id_account"=>array("회계담당자","",100,0,'아아디 입력','',0)
    ,"prj_end_company"=>array("최종고객","",250,0,'','',2)
    ,"prj_content"=>array("의뢰내용","",70,0,'','',0)
    ,"prj_ask_date"=>array("요청날짜","",130,0,'','',2)
    ,"prj_submit_date"=>array("제출날짜","",130,'','','',0)
    ,"prj_belongto"=>array("자사,타사구분","required",130,0,'first=자사, third=타사','',2)
    ,"prj_price"=>array("수주금액","",130,'원','','',0)
    ,"prj_receivable"=>array("미수금","",130,'원','','',2)
    ,"prj_percent"=>array("진행율","",130,'%','진행비율을 숫자로 입력','',0)
);
?>
...
//        print_r2($items1[$k1]).'<br>';
        ...
        // 폼이 다른 구조를 가질 때 재정의
        if(preg_match("/_price$/",$k1)||preg_match("/_receivable$/",$k1)||preg_match("/_percent$/",$k1)) {
