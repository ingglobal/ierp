...
if ($aj == "get") {
	...
	$sql = "SELECT $aj_field
			...
			FROM {$g5['companyreseller_table']} AS com
...
// 회원 리스트
	...
	if (!$aj_auth && $member['mb_level']<8) {
		...
		$sql_join = "	INNER JOIN {$g5['companyreseller_member_table']} AS cmm
							ON cmm.com_idx = com.com_idx " . $sql_my_department . $sql_my_id;
		$sql_groupby = "	GROUP BY com_idx "; 
	}

	$sql_common = " FROM {$g5['companyreseller_table']} AS com {$sql_join} ";
	...
	$sql = "SELECT SQL_CALC_FOUND_ROWS com.*
				...
				, ( SELECT GROUP_CONCAT(CONCAT(mta_key, '=', COALESCE(mta_value, 'NULL'))) FROM {$g5['meta_table']}
					WHERE mta_db_table = 'companyreseller' AND mta_db_id = com.com_idx ) com_metas
			{$sql_common}
			$aj_search
--			GROUP BY com_idx
			$aj_orderby
