...
$('.a_mag').on('click',function(){
	...
	opener.document.getElementById('wr_mb_id_approver').value = $(this).attr('mb_id');
	if($('#mb_rank').length) opener.document.getElementById('mb_rank').value = $(this).attr('mb_rank');
	opener.document.getElementById('mb_approver_name').value = $(this).attr('mb_name');
	window.close();
});
</script>
<?php
