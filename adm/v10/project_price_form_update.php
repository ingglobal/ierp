...
$prj_idx = ($_POST['prj_idx']) ? $_POST['prj_idx'] : '';
/*
...
/*
$_POST['prj_idx']
$_FILES['prp_doc_file']['name']
$_FILES['prp_doc_file']['type']
$_FILES['prp_doc_file']['tmp_name']
$_FILES['prp_doc_file']['error']
$_FILES['prp_doc_file']['size']
...
for($i=0;$i<count($_FILES[$fle_type2.'_file']['name']);$i++) {
	...
	if (${$fle_type2.'_del'}[$i] == 1) {
		if($mb_id) {
			delete_jt_file(array("fle_db_table"=>"prp", "fle_db_id"=>${$pre."_idx"}, "fle_type"=>$fle_type2, "fle_sort"=>$i, "fle_delete"=>1));
		}
		else {
			// fle_db_id를 던져서 바로 삭제할 수도 있고 $fle_db_table, $fle_db_id, $fle_token 를 던져서 삭제할 수도 있음
			delete_jt_file(array("fle_db_table"=>$pre
								,"fle_db_id"=>${$pre."_idx"}
								,"fle_type"=>$fle_type2
								,"fle_sort"=>$i
								,"fle_delete"=>1
...
//exit;
//goto_url('./'.$fname.'_form.php?'.$qstr.'&w=u&g=1&prj_idx='.$prj_idx.'&'.$pre.'_idx='.${$pre."_idx"}, false);
if($grp){
	alert('데이터가 등록되었습니다.','./project_group_price_list.php?'.$qstr, false);
}else{
...
if(!$g){
	...
	goto_url('./project_group_price_list.php?'.$qstr, false);
}
*/
?>