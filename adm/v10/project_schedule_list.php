...
//echo $sql_common;
$where = array();
$where[] = " prs_status NOT IN ('trash','delete') ";   // 디폴트 검색조건
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        // 관리 버튼
        $s_mod = '<a href="./'.$fname.'_form.php?'.$qstr.'&amp;w=u&amp;'.$pre.'_idx='.$row['prs_idx'].'&amp;ser_prj_type='.$ser_prj_type.'&amp;ser_trm_idx_salesarea='.$ser_trm_idx_salesarea.'">수정</a>';
        ...
        $bg = 'bg'.($i%2);
        $prs_status_class = '';
        if($row['prs_status'] == 'hide') $prs_status_class = ' prs_hide';
        else if($row['prs_status'] == 'end') $prs_status_class = ' prs_end';
        // 1번 라인 ================================================================================
        echo '<tr class="'.$bg.$prs_status_class.' tr_'.$row['prj_status'].'" tr_id="'.$row['prj_idx'].'">'.PHP_EOL;
        ?>
		<td class="td_chk" style="display:<?=(!$member['mb_manager_yn'])?'none':''?>;">
			<input type="hidden" name="prs_idx[<?php echo $i ?>]" value="<?php echo $row['prs_idx'] ?>" id="prs_idx_<?php echo $i ?>">
			<label for="chk_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['prs_idx']); ?></label>
        ...
        //    print_r2($items1);
            foreach($items1 as $k1 => $v1) {
                ...
                if($k1=='prj_reg_dt') {
                    ...
                    $row[$k1] = $g5['set_worker_type_value'][$row[$k1]];
                }
                else if($k1=='trm_idx_category') {
                    $row[$k1] = ($row[$k1]) ? $g5['category_name'][$row[$k1]] : '-';
                }
                else if($k1=='prs_task') {
                    $row[$k1] = cut_str($row[$k1],15);
                }
                else if($k1=='prs_status'){
                    $row[$k1] = $g5['set_prs_status_value'][$row[$k1]];
                }
...
<div class="btn_fixed_top">
    <?php if($member['mb_manager_yn']) { ?>
        ...
        <input type="submit" name="act_button" value="선택표시" onclick="document.pressed=this.value" class="btn_02 btn">
        <input type="submit" name="act_button" value="선택숨김" onclick="document.pressed=this.value" class="btn_02 btn">
        <input type="submit" name="act_button" value="선택종료" onclick="document.pressed=this.value" class="btn_02 btn">
