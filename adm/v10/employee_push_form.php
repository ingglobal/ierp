...
function sendMessage1(){
    ...
    $fields = array(
        'app_id' => "49f45cb3-1f2e-473d-b0c2-ee65facbd8bc",
        'include_player_ids' => array(
            "0fb9516e-7805-4db3-88d3-1d4e91096e25"
            ,"12ce6b6b-f80d-4f75-97ad-2a9fd0813f0c"
        ),
        'data' => array("foo" => "bar"),
        'headings' => $headings,
        'contents' => $content
...
function sendMessage2() {
    $content      = array(
        "en" => '이거슨 모든 사람들에게 같이 뿌려지는 것?'
        ,"ko" => '이거 한글로 모든 사람들에게 같이 뿌려지는 것? 영어는 무시?데스?'
    );
    $fields = array(
        'app_id' => "49f45cb3-1f2e-473d-b0c2-ee65facbd8bc",
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        ...
        'Authorization: Basic YWEwNmFmNDItOTQ3YS00Yzc1LWIzYWEtOWE5YWZkNjQ5ZDQ3'
    ));
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
    curl_setopt($ch, CURLOPT_HEADER, FALSE);
    curl_setopt($ch, CURLOPT_POST, TRUE);
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?php echo $bg; ?>" tr_id="<?php echo $row['mb_id'] ?>">
        ...
        <td class="td_manager"><!-- 푸시키 -->
            <?=substr($row['mb_5'],0,4)?>...<?=substr($row['mb_5'],-4)?>
        </td>
...
<div class="btn_fixed_top" style="display:<?php if(auth_check($auth[$sub_menu],'d',1)) echo 'none';?>">
    ...
    <?php if(!auth_check($auth[$sub_menu],'d',1)) { ?>
    <a href="./employee_form.php" id="member_add" class="btn btn_01">관리자추가</a>
    <?php } ?>
