...
$sql_common = " FROM {$g5['companyreseller_table']} AS com 
                LEFT JOIN {$g5['companyreseller_member_table']} AS cmm ON cmm.com_idx = com.com_idx
"; 
...
$g5['title'] = '판매업체 검색'.$total_count_display;
...
<div id="sch_target_frm" class="new_win scp_new_win">
    ...
    <div id="scp_list_find">
        ...
        <input type="text" name="sch_word" id="sch_word" value="<?php echo get_text($sch_word); ?>" class="frm_input required" required size="20" style="width:30%;">
        <input type="submit" value="검색" class="btn_frmline btn btn_10" style="height:35px;line-height:35px;">
        <a href="<?php echo $_SERVER['SCRIPT_NAME']?>?frm=<?php echo $_REQUEST['frm']?>&d=<?php echo $_REQUEST['d']?>" class="btn btn_b10" style="color:#555 !important;">검색취소</a>
    </div>
...
$('.btn_select').click(function(e){
    ...
    ?>
        opener.document.getElementById("com_idx").value = $(this).closest('td').attr('com_idx')
        opener.document.getElementById("com_name").value = $(this).closest('td').attr('com_name')
    <?php
    }
    // 그래프 추가
