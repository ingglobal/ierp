            ...
            <table>
                ...
                <tr>
                    <td colspan="2" style="text-align:center;padding:10px 0;">
                        <button type="button" class="btn btn_01" id="com_reg_btn">기타업체등록</button>
                    </td>
                </tr>
                </tbody>
            </table>
...
$('.a_mag').on('click',function(){
	$(opener.document).find('input[name="com_idx"]').val($(this).attr('com_idx'));
	$(opener.document).find('input[name="com_name"]').val($(this).attr('com_name'));
	window.close();
});
...
function com_reg(com,name,tel){
	var link = '<?=G5_USER_ADMIN_URL?>/_win_companyetc_add_update.php';
	...
	$.ajax({
		...
		success : function(res){
			if(res == 'dp'){
				alert('입력하신 업체명과 유사한 업체가 이미 등록되어 있습니다.\n비과제업체관리 페이지에서 확인하시기 바랍니다.');
				return false;
			}
			else{
				alert('업체를 성공적으로 등록했습니다.\n상세정보는 [비과제업체관리]에서 수정하시기 바랍니다.');
				location.reload();
			}
		},
