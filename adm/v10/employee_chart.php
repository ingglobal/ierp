...
$sql = " SELECT mb_id, mb_name, mb_1, mb_2, mb_3 AS mb_rank
			...
			WHERE mb_level IN (5,6,7,8) AND mb_2 != '' AND mb_leave_date = '' AND mb_id NOT IN ('".implode("','",$mb_excludes)."')
			{$sql_search_mb}
...
$g5['title'] = '조직도 ('.number_format($total_count).'명)';
include_once('./_top_menu_employee.php');
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
	...
	$list[$i]['field'] = $row['term_name'].'<br>'.$row['mb_count'].'명'.$list[$i]['mb_count1_text'].' <a href="./employee_list.php?ser_trm_idxs='.$row['down_idxs'].'" style="font-size:0.8em;">List</a>';	// 항목표현내용
...
<div class="btn_fixed_top" style="display:none;">
	<a href="javascript:" class="btn btn_02" id="employee_cart_popup">크게보기</a>
</div>
