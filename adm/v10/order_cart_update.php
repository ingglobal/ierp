...
if($act == 'order'){
    ...
    */
    ...
    goto_url('./order_form.php?od_id='.$od_id.'&amp;uid='.$uid);
}
else if($act == "buy")
{
    ...
    if ($is_member) // 회원인 경우
        goto_url('./order_cart_form.php');
    else
        goto_url(G5_BBS_URL.'/login.php?url='.urlencode(G5_USER_URL.'/intra/order_cart_form.php'));
}
...
// 바로 구매일 경우
if ($sw_direct)
	goto_url("./order_cart_form.php?sw_direct=$sw_direct");
else {
	...
	?>
	<script>
        location.href = "<?php echo "./order_cart.php"; ?>";
        /*
		if (confirm("장바구니에 상품을 담았습니다.\n계속해서 상품을 담으시겠습니까?"))
			location.href = "<?php echo "./order_product_select.php?".str_replace('&amp;', '&', $qstr); ?>";
		else
			location.href = "<?php echo "./order_cart.php"; ?>";
        */
	</script>
