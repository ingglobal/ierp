...
if ($stx) {
    switch ($sfl) {
		case ( $sfl == 'prx.com_idx' || $sfl == 'prx.prj_idx' || $sfl == 'prx_idx') :
            $where[] = " ({$sfl} = '{$stx}') ";
            break;
...
$sql = " SELECT SQL_CALC_FOUND_ROWS prx.*
            , com_name
            , prj_name
            , (SELECT prp_price FROM {$g5['project_price_table']} WHERE prj_idx = prx.prj_idx AND prp_type = 'order' AND prp_status = 'ok' ) AS prp_order_price
            , (SELECT SUM(prn_price) FROM {$g5['project_inprice_table']} WHERE prj_idx = prx.prj_idx AND prn_status = 'ok' AND prn_done_date != '0000-00-00') AS prn_sum_inprice
            , (SELECT SUM(prx_price) FROM {$g5['project_exprice_table']} WHERE prj_idx = prx.prj_idx AND prx_status = 'ok' AND prx_done_date != '0000-00-00' ) AS prx_don_exprice
            , (SELECT SUM(prx_price) FROM {$g5['project_exprice_table']} WHERE prj_idx = prx.prj_idx AND prx_status = 'ok' ) AS prx_sum_exprice
            , (
            ...
            , ( SELECT MAX(prx_idx) FROM {$g5['project_exprice_table']} WHERE prj_idx = prx.prj_idx ) AS prx_max_idx
        {$sql_common}
		{$sql_search}
        {$sql_order}
...
<select name="sfl" id="sfl">
	...
	<option value="prx.prj_idx"<?php echo get_selected($_GET['sfl'], "prx.prj_idx"); ?>>프로젝트번호</option>
</select>
...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
        ...
        <th scope="col">기타수입금액<br>(충수입금기준%)</th>
        <th scope="col">총수입합계</th>
        <?php } ?>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        // print_r2($row);
        // 관리 버튼
        $s_mod = '<a href="./project_expense_form.php?'.$qstr.'&amp;w=u&amp;prj_idx='.$row['prj_idx'].'&amp;ser_prj_type='.$ser_prj_type.'&amp;ser_trm_idx_salesarea='.$ser_trm_idx_salesarea.'&amp;group=1&amp;divid=divid">수정</a>';
        $row['tot_income_price'] = $row['prp_order_price'] + $row['prn_sum_inprice'];
        $row['prp_dif_exprice'] = ($row['tot_income_price']) - $row['prx_sum_exprice'];
        $bg = 'bg'.($i%2);
        $inp_per = ($row['prp_order_price'])?round($row['prn_sum_inprice']/$row['tot_income_price']*100,1):0;
        $exp_per = ($row['prp_order_price'])?round($row['prx_sum_exprice']/($row['tot_income_price'])*100,1):0;
        $dif_per = ($row['prp_order_price'])?round($row['prp_dif_exprice']/($row['tot_income_price'])*100,1):0;
        $tex_per = ($row['prx_sum_exprice'])?round($row['prx_price']/($row['prx_sum_exprice'])*100,1):0;

        $prj_last = ($row['prx_idx'] == $row['prx_max_idx']) ? ' tr_last' : '';
        ?>
        <tr class="<?=$bg.$prj_last?>">
            ...
            <td rowspan="<?=$p_cnt?>" style="text-align:right;width:110px;">
                <?=number_format($row['tot_income_price'])?>
            </td>
            <?php } ?>
            <td rowspan="<?=$p_cnt?>" class="td_grp" style="text-align:right;width:110px;">
            ...
            <td rowspan="<?=$p_cnt?>" class="td_grp td_alarm" style="text-align:right;width:100px;">
                ...
                <?php if($super_admin){ ?><div class="grp_box"><div class="grp_in" style="width:<?=$tex_per?>%"></div></div><?php } ?>
                <?php if($super_admin){ ?><span class="per">(<?=$tex_per?>%)</span><?php } ?>
                <?php
